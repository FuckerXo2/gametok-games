window.zs = window.zs || {}, window.zs.laya = window.zs.laya || {},
    function(t, s) {
        ...
        class i {
            ...
            static binl2b64(t) {
                for (var s = "", i = 0; i < 4 * t.length; i += 3)
                    for (var e = (t[i >> 2] >> i % 4 * 8 & 255) << 16 | (t[i + 1 >> 2] >> (i + 1) % 4 * 8 & 255) << 8 | t[i + 2 >> 2] >> (i + 2) % 4 * 8 & 255, h = 0; h < 4; h++) 8 * i + 6 * h > 32 * t.length ? s += this.b64pad : s += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(e >> 6 * (3 - h) & 63);
                return s;
            }
        }
        ...
        class h {
            ...
            static requestLoginByCode(t, s) {
                var i = s || {};
                i.code = t, i.timestamp = Date.now(), this.UseWebApi && (this.RequestSign ?
                    zs.laya.XHRUtils.xhrPostWithSign(this.WebApiMap.login, i, this.RequestSign) : zs.laya.XHRUtils.xhrPost(this.WebApiMap.login, i),
            ...
            static requestLoginByUserId(t, s) {
                ...
                i.user_id = t, i.timestamp = Date.now(), this.UseWebApi && (this.RequestSign ?
                    zs.laya.XHRUtils.xhrPostWithSignAndHeader(this.WebApiMap.login, i, this.RequestSign, this.RequestHeader) : zs.laya.XHRUtils.xhrPost(this.WebApiMap.login, i),
                    console.log(this.WebApiMap.login));
            }
